...
export const PROJECTS: ProjectList[] = [
  [
    <div key={"사이드 퀘스트"}>
      ...
      앞으로 일어날 성능적인 이슈들에 대비해 우리 팀이 사용할 스택들을 미리
      정하여 프로젝트를 시작했고,{" "}
      <FocusSpan content="사용자의 입장에서 끊임없이 고민하며, UX를 향상하려고 노력하였습니다." />
      <p />
      <b>
        UX를 향상하기 위해 제가 한 노력은,
        <div>
          1. 모든 API 서버 데이터들을 React Query 라이브러리를 통해 캐싱
        </div>
        <div>
          2. Refetch를 적극 활용하여 양방향 통신을 하는 듯한 느낌을 주려고 노력
        </div>
        ...
        <div>
          5. 프로젝트의 특성상 많은 이미지가 사용되는 구조여서, 클라이언트에서
          관리되는 이미지들은 최대한 스트라이프화
        </div>
      </b>{" "}
      <p />
      <FocusSpan content="프로젝트 기간에는 팀장으로서 책임감을 느끼고 다른 사람들이 힘들어하는 부분들을 도맡아 진행하였고, 프로젝트 기간이 끝난 후에도, 미흡한 부분들을 끝까지 해결하고 리팩토링을 통해 성능을 높이려는 노력을 많이 하였습니다." />
    </div>,
    [
